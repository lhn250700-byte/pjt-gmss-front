...
const KakaoAdditionalInfo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nickname || !formData.birthdate) {
      setError('닉네임과 생년월일은 필수 입력 항목입니다.');
      return;
    }

    const birthRegex = /^\d{8}$/;
    if (!birthRegex.test(formData.birthdate.replace(/-/g, ''))) {
  ...
  const handleDuplicateCheck = () => {
    // TODO: 백엔드/ Supabase 연동 시 닉네임 중복 확인 API 호출
    alert('닉네임 중복 확인 기능은 추후 구현 예정입니다.');
  };

  // 카카오 첫 로그인 사용자만 이 페이지 접근 가능
  useEffect(() => {
    ...
    if (user.provider !== 'kakao' || user.kakao_additional_done) {
      navigate('/', { replace: true });
    }
  }, [authLoading, user.isLogin, user.provider, user.kakao_additional_done, navigate]);
  ...
  if (!user.isLogin || user.provider !== 'kakao' || user.kakao_additional_done) {
    return null;
  }
    ...
    <div className="relative w-full min-h-screen bg-[#f3f7ff] flex items-center justify-center p-0 lg:p-8">
      <div className="w-full max-w-[390px] lg:max-w-[500px] min-h-screen lg:min-h-0 lg:overflow-y-auto mx-auto bg-[#f3f7ff] lg:bg-white lg:rounded-3xl lg:shadow-xl">
        <div className="px-6 pt-6 pb-28 lg:px-12 lg:py-10">
          ...
          <p className="text-xs text-red-600 mb-4">*표시 사항은 필수 입력 항목</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 lg:gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold mb-2 text-gray-700">
                자기 소개(페르소나) <span className="text-blue-600">(선택)</span>
              </label>
              <textarea
                name="introduction"
