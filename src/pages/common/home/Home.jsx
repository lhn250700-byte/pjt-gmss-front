  ...
  useEffect(() => {
    const fetchPopularPosts = async () => {
      try {
        ...
        // 데이터가 있을 때만 업데이트, 없으면 빈 배열로 유지
        if (data) {
          setCommunityTopPosts(data);
        } else {
          // 기본 샘플 데이터 혹은 빈 배열
          setCommunityTopPosts([
            { bbsId: 1, title: "게시글이 없습니다.", bbsLikeCount: 0 },
          ]);
        }
      } catch (error) {
        console.error("인기글 로딩 실패:", error);
        setCommunityTopPosts([]); // 에러 시 빈 배열로 안전하게 처리
      }
    };
    ...
    apiCall
      .then((res) => {
        ...
        if (communityMode === "recommend" && items.length > 0) {
          items = [...items].sort(
            (a, b) => (b.postScore ?? 0) - (a.postScore ?? 0),
          );
        }
