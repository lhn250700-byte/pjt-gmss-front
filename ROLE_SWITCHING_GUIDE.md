...
### 상담사로 로그인했는데 상담 내역 리스트가 비어 있을 때:
더미 계정은 **Supabase DB에 회원/상담 데이터가 없으면** 목록이 비어 있습니다.  
프로젝트 루트의 **`docs/DUMMY_COUNSELOR_SEED.sql`** 을 Supabase SQL Editor에서 실행하면,  
`counselor@test.com` 상담사용 테스트 상담 예약·내역이 들어가서 리스트에 표시됩니다.
